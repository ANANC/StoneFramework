...
namespace LitJson
{
    ...
    public class JsonReader
    {
        ...
        private void ProcessNumber (string number,Type inst_type)
        {
            if (number.IndexOf ('.') != -1 ||
                number.IndexOf ('e') != -1 ||
                number.IndexOf ('E') != -1) {
            ...
            //add by eric 当比较明确是需要什么目标类型时，就明确使用相应的类型进行解释
            //不然有时候int32转int64需要要显示强转就转不了
            if(inst_type != null)
            {
                
                if(inst_type == typeof(Int64))
                {
                    Int64 n_int64t = 0;
                    if (Int64.TryParse(number, out n_int64t))
                    {
                        token = JsonToken.Long;
                        token_value = n_int64t;
                        return;

                    }
                }

                else if (inst_type == typeof(Int32))
                {
                    Int32 n_int32t = 0;
                    if (Int32.TryParse(number, out n_int32t))
                    {
                        token = JsonToken.Int;
                        token_value = n_int32t;
                        return;

                    }
                }
                else if (inst_type == typeof(UInt64))
                {
                    UInt64 n_int64t = 0;
                    if (UInt64.TryParse(number, out n_int64t))
                    {
                        token = JsonToken.Long;
                        token_value = n_int64t;
                        return;
                ...
                else if (inst_type == typeof(UInt32))
                {
                    UInt32 n_int32t = 0;
                    if (UInt32.TryParse(number, out n_int32t))
                    {
                        token = JsonToken.Int;
                        token_value = n_int32t;
        ...
        private void ProcessSymbol (Type inst_type = null)
        {
            if (current_symbol == '[')  {
                ...
                ProcessNumber (lexer.StringValue, inst_type);
        ...
        public bool Read (Type inst_type = null)
        {
            if (end_of_input)
                return false;
