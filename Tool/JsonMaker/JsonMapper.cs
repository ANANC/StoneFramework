...
namespace LitJson
{
    ...
    public class LitJsonObjectAttribute : Attribute
    {
        public LitJsonObjectAttribute()
        {
        }

        public static implicit operator bool(LitJsonObjectAttribute attribute)
        {
            return attribute != null;
        }

        public bool NoSelfType { get; set; }
    ...
    public class JsonMapper
    {
                                        ...
                                        int depth)
        {
            ...
            writer.WriteObjectStart ();
            writer.WritePropertyName("selfType");
        ...
        {
            if(null != namespaces) namespaces.Clear();
        }


        //名字后面必须带点
        public static void AddNamespace(string namesspace)
        {
            if (null == namespaces)
            {
                namespaces = new List<string>();
                namespaces.Add("");//一定有一个空的
            }

            if (namespaces.Contains(namesspace)== false)
            {
                namespaces.Add(namesspace);
            }
        }
		...
		public static List<T> ToObjects<T> (string json) 
		{
			...
			JsonData item = ToObject(reader);
			
			if (item != null && item.Count > 0) {
				List<T> vars = new List<T>();
				for(int i = 0; i < item.Count; i++) {
					vars.Add(ToObject<T>(item[i].ToString()));
				}
				return vars;
			}
			return null;
